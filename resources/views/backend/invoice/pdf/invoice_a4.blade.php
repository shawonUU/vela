...
<head>
    ...
    <title>Invoice #{{$invoice->id}} - Printed At: {{ now()->format('d-m-Y  h:i A') }} - Printed By: {{ Auth::user()->name }}</title>
...
<body class="d-flex flex-column min-vh-100" style="margin-left: 42px;">
    ...
    <div class="invoice-container flex-fill" id="printableArea">
        ...
        <div class="invoice-details" style="font-size: 1rem;  border-top: 2px solid #000;">
            <div class="mt-2" style="text-align: start; line-height: 1;">
                <p class="mb-1"><strong>{{ $payment['customer']['name'] ??"N/A"}}</strong></p>
                <p class="mb-1">Email: {{$payment['customer']['email'] ??'N/A' }}</p>
                <p class="mb-1">Phone: {{$payment['customer']['mobile_no'] ??'N/A' }}</p>
                <p class="mb-1">Address: {{$payment['customer']['office_address'] ??'N/A' }}</p>
        ...
        @php
        if($invoice['invoice_details']->count()>6){
        $firstPageChunk = collect($invoice['invoice_details'])->take(12);
        $remainingChunks = collect($invoice['invoice_details'])->skip(12)->chunk(13);
        }
        @endphp
        @if ($invoice['invoice_details']->count()>6)
        ...
        <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size: 1rem;">
            <thead style=" color: black; font-weight: bold;">
                <tr style=" color: black; font-weight: bold;">
                    ...
                    <th style="padding: 8px 12px; text-align: left;">Product (Size)</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #000;">
                    @php
                        ...
                        $productSize = $product_size ? $product_size->size->name : 'Unknown Product';
                        $productDescription = $product ? $product->description : 'No description available';
                        $productBrand = $product && $product->brand ? $product->brand->name : 'N/A';
                        $productUnit = $product ? $product->unit->name : 'N/A';
                        $productId = $product ? $product->id : 'N/A';
                        $productImage = $product ? asset($product->image) : asset('backend/assets/images/no-image.png');
                    @endphp
                    <td style="padding: 8px 12px;">{{ $loop->iteration }}</td>
                    <td style="padding: 8px 12px; text-align: left;">
                        <span style=" color: black; font-weight: bold;">{{ $productName.' ('.$productSize.')'}}</span>
                        <br>{!! $productDescription !!}
                    </td>
                    <td style="padding: 8px 12px; text-align: center;">{{ $productBrand }}</td>
                    <td style="padding: 8px 12px; text-align: center;">{{ $details->selling_qty }}</td>
                    <td style="padding: 8px 12px; text-align: center;">{{ $productUnit}}</td>
        ...
        <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size:1rem;">
            ...
            <tbody>
                ...
                <tr style="font-weight: 400; ">
                    ...
                    <td class="text-end">{{ number_format($invoice->invoice_discount_amount, 2) }}</td>
                </tr>
                @endif
                ...
                <tr style="font-weight: 400; ">
                    ...
                    <td class="text-end">{{ number_format($total_selling_price-$invoice->invoice_discount_amount, 2) }}</td>
                </tr>
                @endif
                ...
                @endif
                @php
                $invoice_tax_ids = json_decode($invoice->invoice_tax_type, true);
                $taxes = \App\Models\Tax::whereIn('id', $invoice_tax_ids)->get();
                $taxList = $taxes->map(fn($tax) => "{$tax->name}")->implode(', '); // Format & join with comma
                @endphp
                @if (count($invoice_tax_ids)>0)
                <tr style="font-weight: 400; padding:0;">
                    <td colspan="7" class="thick-line text-end">
                        <strong>Invoice VAT({{$taxList}})</strong>
                    </td>
                    <td class="text-end">{{ number_format($invoice->invoice_tax_amount, 2) }}</td>
                </tr>
                @endif
                ...
                @endif
                @php
                $invoice_additional_charge_ids = json_decode($invoice->additional_charge_type, true);
                ...
                @endphp
                @if (count($invoice_additional_charge_ids) > 0)
                <tr style="font-weight: 400; padding:0;">
                    <td colspan="7" class="thick-line text-end">
        ...
        @if ($invoice['invoice_details']->count()>6 && !$remainingChunks->isNotEmpty())
        <div class="page-break"></div>
        <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size: 1rem;">
            <thead style=" color: black; font-weight: bold;">
                <tr style=" color: black; font-weight: bold;">
        ...
        <div class="total-section my-0 py-0">
            <div class="total-row" style="display: flex; justify-content: space-between; font-size: 0.7rem;">
                <span style=" width: 80%; display: inline-block;">
                    <b>IN WORDS: </b><strong>{{ strtoupper(convertToBangladeshiWords($payment->paid_amount)) }}</strong>
                </span>
    ...
    <footer class="text-center mt-1 text-muted d-flex justify-content-center px-3"
        ...
        <small>{{ $org->org_name_en??'N/A' }} | Invoice No: {{$invoice->id}} | Generated By: {{ Auth::user()->name }} | At: {{ now()->format('d-m-Y  h:i A') }}</small>
    </footer>
</body>
