...
<head>
  ...
  <title> {{ !empty($customer->name) ? $customer->name :"" }} Invoices Report - Printed At: {{ now()->format('d-m-Y  h:i A') }} - Printed By: {{ Auth::user()->name }}</title>
...
<body class="d-flex flex-column min-vh-100" style="margin-left: 42px;">
  ...
  <div class="invoice-container flex-fill" id="printableArea">
    ...
    <h3 style="text-align: center; margin: 0rem 0rem;"><b>Invoices</b></h3>
    ...
    <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size: 0.6rem;">
      ...
      <tbody>
        ...
        <tr>
          <td>{{ $key + 1 }}</td>
          <td><a href="{{ route('customer.edit.invoice', $item->invoice_id) }}">#{{ $item->invoice_id }}</a></td>
