    ...
    <div id="layout-wrapper">
            ...
            <div class="page-content">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0">Edit Invoice ({{$invoice->id}})</h4>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-8 col-s-12">
                            <div class="card">
                                <form method="post" action="{{ route('invoice.update') }}" onsubmit="return confirmAction(event)">
                                    @csrf
                                    <div class="card-body pb-0">
                                        <div class="row pb-2">
                                            <div class="col-md-6 text-left">
                                                <label for="example-text-input" class="col-form-label">Invoice No.</label>
                                                <input class="form-control" type="text" name="invoice_no" value="{{ $invoice->id }}" id="invoice_no" readonly style="background-color:#ddd; ">
                                            </div>
                                            {{--
                                                <div class="form-group col-md-5 pb-0">
                                                    ...
                                                    <select name="customer_id" id="customer_id" class="form-select">
                                                        ...
                                                        $customer_selected_id = \App\Models\Customer::find($payment->customer_id);
                                                        @endphp
                                                        @if (empty($customer_selected_id))
                                                        <option value="{{$payment->customer_id}}" selected>Walking Customer</option>
                                            @foreach($customers as $customer)
                                            <option value="{{ $customer->id }}">{{ $customer->name }} {{$customer->mobile_no?'-'. $customer->mobile_no:''}}</option>
                                            @endforeach
                                            @else
                                            <option value="0">Walking Customer</option>
                                            @foreach($customers as $customer)
                                            <option value="{{ $customer->id }}" {{ $payment->customer_id == $customer->id ?'selected': '' }}>{{ $customer->name }} {{$customer->mobile_no?'-'. $customer->mobile_no:''}}</option>
                                            @endforeach
                                            @endif
                                            </select><br>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input class="form-control example-date-input" name="date" value="{{ $invoice->date }}" type="date" id="date">
                                        </div>
                                    ...
                                    <div class="row mb-0">
                                        ...
                                        $customer_selected = \App\Models\Customer::find($payment->customer_id);
                                        @endphp
                                        <!-- Customer Name -->
                                        <div class="form-group col-md-4">
                                            <input type="text" name="name" id="name" class="form-control" placeholder="Customer Name" value="{{$customer_selected->name??''}}">
                                        </div>

                                        <!-- Mobile with Autocomplete -->
                                        <div class="form-group col-md-4 position-relative">
                                            <input type="text" name="mobile_no" id="mobile_no" class="form-control" placeholder="Customer Mobile No" autocomplete="off" value="{{$customer_selected->mobile_no??''}}">
                                        ...
                                        <!-- Customer Email -->
                                        <div class="form-group col-md-4">
                                            <input type="email" name="email" id="email" class="form-control" placeholder="Customer Email" value="{{$customer_selected->email??''}}">
                                        </div>
                                    </div>
                            ...
                            <div class="card-body">
                                <table class="table-sm table-bordered" width="100%" style="border-color: #ddd;">
                                    ...
                                    <tbody id="addRow" class="addRow">
                                        @foreach ($invoice['invoice_details'] as $key => $invoice_details)
                                        <tr class="delete_add_more_item" id="delete_add_more_item">
                                            <input type="hidden" name="date" value="{{$invoice_details->date?? ''}}">
                                            <td class="serial-number">{{ $key+1 }}</td>
                                            <input type="hidden" name="invoice_no" value="{{$invoice->id ?? ''}}">
                                            <input type="hidden" name="invoice_details_id[]" value="{{$invoice_details->id ?? ''}}">
                                            <input type="hidden" name="category_id[]" value="{{$invoice_details->category_id ?? ''}}">
                                            <input type="hidden" name="product_size_id[]" value="{{$invoice_details->product_id ?? ''}}">
                                            @php
                                            $product_size = \App\Models\ProductSize::with('product')->find($invoice_details->product_id);
                                            @endphp
                                            <td>{{ $product_size['product']->name ?? 'N/A' }}{{!empty($product_size['size']->name)?'('.$product_size['size']->name.')':''}}{{!empty($product_size['product']['brand']->name ) ?'-'.$product_size['product']['brand']->name: ''}}</td>
                                            <td>
                                                <input type="number" min="1" class="form-control selling_qty text-right" name="selling_qty[]" id="selling_qty" value="{{$invoice_details->selling_qty}}">
                                                <input type="number" class="form-control stock_quantity text-right" value="{{$product_size->quantity??0}}" readonly>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control product_buying_price text-right" name="product_buying_price[]" value="{{$product_size->buying_price}}" hidden>
                                                <input type="text" class="form-control unit_price text-right" name="unit_price[]" value="{{$invoice_details->unit_price}}">
                                                <input type="text" class="form-control unit_price_code text-right" name="product_price_code" value="{{$product_size->buying_price_code}}" readonly>
                                            </td>

                                            <!-- Discount  -->
                                            <td class="discount-row">
                                                <div class="form-check form-switch">
                                                    <input class="form-check-input discount_per_product" type="checkbox" {{ $invoice_details->discount_type == 'fixed' ? 'checked' : '' }}>
                                                    <label class="form-check-label discount_label" for="flexSwitchCheckDefault">{{ $invoice_details->discount_type == 'fixed' ? 'Fixed' : '% Percent' }}</label>
                                                </div>
                                                <input type="text" class="form-control discount_rate text-right" id="discount_rate" name="discount_rate[]" value="{{$invoice_details->discount_rate}}" autocomplete="off">
                                                <input type="text" class="form-control discount_amount_per_product text-right" id="discount_amount_per_product" name="discount_amount_per_product[]" value="{{$invoice_details->discount_amount}}" readonly>
                                            </td>
                                            <td>
                                                <input type="text" class="form-control selling_price text-right" id="selling_price" name="selling_price[]" value="{{$invoice_details->selling_price}}" readonly>
                                                @php
                                                $buying_price = $product_size->buying_price * $invoice_details->selling_qty;
                                                @endphp
                                                <input type="text" class="form-control buying_price text-right" id="buying_price" name="buying_price[]" value="{{$buying_price}}" readonly hidden>
                                    ...
                                    <tbody>
                                            ...
                                            <td colspan="5" class="text-end">
                                                <div class="form-group">
                                                    <select name="discount_status" id="discount_status" class="form-select" style="float: right; width:31%">
                                                        <option value="fixed_discount" {{ $invoice->invoice_discount_type === 'fixed_discount' ? 'selected':'' }}>Fixed Discount</option>
                                                        <option value="percentage_discount" {{ $invoice->invoice_discount_type === 'percentage_discount' ? 'selected':'' }}>Percentage Discount(%)</option>
                                                    </select>
                                                </div>
                                            </td>
                                            <td>
                                                <input type="text" name="discount_amount" id="discount_amount" class="form-control discount_amount" value="{{$invoice->invoice_discount_type === 'percentage_discount' ? $invoice->invoice_discount_rate : $invoice->invoice_discount_amount }}" autocomplete="off">
                                                <input type="text" name="discount_show" id="discount_show" class="form-control discount_show" value="{{$invoice->invoice_discount_type === 'percentage_discount' ? $invoice->invoice_discount_amount : $invoice->invoice_discount_rate }}" readonly>
                                            </td>
                                        <tr>
                                            <td colspan="5" class="text-end">
                                                Total Discount
                                        ...
                                        <tr>
                                            ...
                                            <td class="align-middle">
                                                <input type="text" name="total" value="{{$payment->total_amount ?? 0}}" id="total" class="form-control form-control text-start bg-light" readonly>
                                            </td>
                                        </tr>
                                        <tr>
                                            {{--
                                        ...
                                        --}}
                                            <td colspan="5" class="text-start">
                                                <div class="row g-2" style="padding-right: 100px;">
                                                    ...
                                                    {{--
                                                        <div class="col-md-3">
                                                            ...
                                                            <input type="text" name="cash" id="cash" class="form-control cash" value="{{$payment->cash}}" placeholder="Cash" autocomplete="off">
                                                </div>
                                                --}}
                                                <div class="col-md-3">
                                                    <label for="bkash" class="form-label">bkash</label>
                                                    <input type="text" name="bkash" id="bkash" class="form-control bkash" value="{{$payment->bkash}}" placeholder="bkash" autocomplete="off">
                                                </div>

                                                <div class="col-md-3">
                                                    <label for="nagad" class="form-label">nagad</label>
                                                    <input type="text" name="nagad" id="nagad" class="form-control nagad" value="{{$payment->nagad}}" placeholder="nagad" autocomplete="off">
                                                </div>        
                                                <!-- Card Payments -->
                                                <div class="col-md-3">
                                                    <label for="visa_card" class="form-label">Visa Card</label>
                                                    <input type="text" name="visa_card" id="visa_card" class="form-control visa_card" value="{{$payment->visa_card}}" placeholder="Visa" autocomplete="off">
                                                </div>

                                                <div class="col-md-3">
                                                    <label for="master_card" class="form-label">Master Card</label>
                                                    <input type="text" name="master_card" id="master_card" class="form-control master_card" value="{{$payment->master_card}}" placeholder="MasterCard" autocomplete="off">
                                                </div>
                                                ...
                                                <div class="col-md-3">
                                                    ...
                                                    <input type="text" name="rocket" id="rocket" class="form-control rocket" value="{{$payment->rocket}}" placeholder="rocket" autocomplete="off">
                                                </div>

                                                <div class="col-md-3">
                                                    <label for="upay" class="form-label">upay</label>
                                                    <input type="text" name="upay" id="upay" class="form-control upay" value="{{$payment->upay}}" placeholder="upay" autocomplete="off">
                                                </div>

                                                <div class="col-md-3">
                                                    <label for="surecash" class="form-label">surecash</label>
                                                    <input type="text" name="surecash" id="surecash" class="form-control surecash" value="{{$payment->surecash}}" placeholder="surecash" autocomplete="off">
                                                </div>

                                                <div class="col-md-3">
                                                    <label for="online" class="form-label">Online</label>
                                                    <input type="text" name="online" id="online" class="form-control online" value="{{$payment->online}}" placeholder="online" autocomplete="off">
                                                </div>
                            </div>
                            </td>
                            ...
                            <td>
                                ...
                                <input type="text" name="paid_amount" id="paid_amount" class="form-control paid_amount" value="{{$payment->paid_amount}}" readonly placeholder="Paid" autocomplete="off">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-12">
                                    <textarea name="description" id="description" class="form-control" placeholder="Write Description Here...">{{$invoice->description??''}}</textarea>
                                </div>
                            </div> <br>
                            ...
                            <div class="form-group">
                                ...
                                <button type="submit" name="saveBtn" value="2" class="btn btn-primary" id="saveAndPrintPDFButton"><i class="fas fa-check-circle"></i> Invoice</button>
                            </div>
                        </div> <!-- End card-body -->
                        </form>
                    </div>
        ...
        <script id="document-template" type="text/x-handlebars-template">
            <tr class="delete_add_more_item" id="delete_add_more_item">
                    ...
                    <input type="hidden" name="invoice_details_id[]" value="0">
            ...
            $(document).ready(function() {
                $('#mobile_no').on('input', function() {
                    ...
                    let matches = customers.filter(c => {
                        return c.mobile_no.toLowerCase().includes(query) ||
                            query.includes(c.mobile_no.toLowerCase());
                    });
        ...
        <script>
            $(document).ready(function() {
                ...
                //round into follor value of grand total
                // let roundEnabled = false; // default: no rounding

                // $('#round_btn').on('click', function() {
                //     roundEnabled = !roundEnabled;

                //     // Update button text and style
                //     $(this).text(roundEnabled ? 'Disable Round' : 'Enable Round');
                //     $(this).toggleClass('btn-success btn-secondary');

                //     // Recalculate totals with new rounding setting
                //     adjustPaidAmount();
                // });
                ...
                function updateProductRow(row) {
                    ...
                    // alert(totalTax* unitPrice * quantity);
        ...
        <script type="text/javascript">
            function total_buying_price_code_generator(id) {
                ...
                var html = "";
                var numberDictionary = @json($product_price_code -> pluck('code', 'number'));
