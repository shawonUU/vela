...
<head>
    ...
    <title>Purchase Report - Printed At: {{ now()->format('d-m-Y  h:i A') }} - Printed By: {{ Auth::user()->name }}</title>
...
<body class="d-flex flex-column min-vh-100" style="margin-left: 42px;">
    ...
    <div class="invoice-container flex-fill" id="printableArea">
        ...
        <div class="invoice-details" style="font-size: 0.8rem;">
            ...
            @php
                $supplier = App\Models\Supplier::findOrFail($supplier_filter);
            @endphp
            <div class="text-end mt-2">
                <p class="mb-1"><strong>{{ $supplier->id != -1 ? $supplier->name : "পথচারি কাস্টমার"}}</strong></p>
                <p class="mb-1">Email: {{$supplier->id != -1 ? $supplier->email :'N/A' }}</p>
                <p class="mb-1">Phone: {{$supplier->id != -1 ? $supplier->mobile_no :'N/A' }}</p>
                @if ($supplier->office_address && $supplier->id != -1)
                    <p class="mb-1">Address: {{$supplier->id != -1 ? $supplier->office_address :'N/A' }}</p>
                @endif
            </div>
            @endif
        </div>
        <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size: 0.6rem;">
            <thead style="background-color: #dc3545; color: black; font-weight: bold;">
                <tr style="background-color: #dc3545; color: black; font-weight: bold;">
                    ...
                    <th style="padding: 8px 12px; text-align: left;">Purchase No</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #000;">
                    ...
                    <td style="padding: 8px 12px; text-align: left;">{{ ($item['supplier_purchese_payment']['supplier_id'] != -1)? $item['supplier_purchese_payment']['supplier']['name']: 'পথচারি কাস্টমার' }}</td>
                    <td style="padding: 8px 12px; text-align: left;">#{{ $item->purchase_no }}</td>
                    <td style="padding: 8px 12px; text-align: center;">{{ date('d-m-Y',strtotime($item->date)) }}</td>
                    <td style="padding: 8px 12px; text-align: right;">{{ number_format($item['supplier_purchese_payment']['due_amount'],2) }} Tk</td>
                    <td style="padding: 8px 12px; text-align: right;">{{ number_format($item['supplier_purchese_payment']['total_amount'],2) }} Tk</td>
                </tr>
                @endforeach
