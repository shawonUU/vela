...
<head>
  ...
  <title>Transactions Report - Printed At: {{ now()->format('d-m-Y  h:i A') }} - Printed By: {{ Auth::user()->name }}</title>
...
<body class="d-flex flex-column min-vh-100" style="margin-left: 42px;">
  <script>
    ...
    function performAction() {
      window.location.href = '/purchase/supplier/transaction';
    }
  </script>
  <div class="invoice-container flex-fill" id="printableArea">
    ...
    <div class=" mt-2" style="text-align: start; line-height: 1.2;">
      <p class="mb-1"><b>From: </b></p>
      {{ !empty($show_start_date) ? date('d-m-Y', strtotime($show_start_date)) : 'N/A' }}
    </div>
    <div class="text-end mt-2">
      <p class="mb-1"><b>To: </b></p>
      {{ !empty($show_end_date) ? date('d-m-Y', strtotime($show_end_date)) : 'N/A' }}
    </div>

    @if ($supplier_filter !="null")
    @php
    $supplier = \App\Models\Supplier::where('id', $supplier_filter)->first();
    @endphp
    <div class=" mt-2" style="text-align: start; line-height: 1.2;">
      <p class="mb-1"><b>{{ !empty($supplier->name) ? $supplier->name :"N/A" }}</b></p>
      ...
      <br>
      <p class="mb-1"><b>Office Address: </b></p> {{ !empty($supplier->office_address) ? $supplier->office_address :"N/A" }}<br>
    </div>
    <div class="text-end mt-2">
      <p class="mb-1"><b>Factory Address: </b></p> {{ !empty($supplier->factory_address) ? $supplier->factory_address :"N/A" }}<br>
      <p class="mb-1"><b>Contact Person Name: </b></p> {{ !empty($supplier->contact_person_name) ? $supplier->contact_person_name :"N/A" }}<br>
      <p class="mb-1"><b>Contact Person Phone: </b></p> {{ !empty($supplier->contact_person_phone) ? $supplier->contact_person_phone :"N/A" }}<br>
    </div>
    @endif
  </div>
  <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size: 0.6rem;">
    ...
    <tbody>
      ...
      <tr>
        <td>{{ $key + 1 }}</td>
        @if ($supplier_filter =="null")
        <td>{{ $item['supplier_name'] }}</br>
          {{ $item['supplier_phone'] }}</br>
        </td>
        @endif
        <td>
          @foreach ($purchaseIdsArray as $purchaseId)
          <a href="{{ route('purchase.edit', $purchaseId) }}">#{{ $purchaseId }}</a>
          @endforeach
        </td>
      ...
      <tr>
        @php
          $col_span = $supplier_filter == "null" ? 6 : 5;
        @endphp
        <th colspan="{{$col_span}}" class="text-end" style="color: #000;">Total Paid Amount:</th>
        <th style="color: #000;">৳ {{ number_format($total_paid, 2) }} Tk</th>
      </tr>
    </tbody>
  </table>
  </div>
