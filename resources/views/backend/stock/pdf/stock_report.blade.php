...
<head>
  ...
  <title> Stock Report - Printed At: {{ now()->format('d-m-Y  h:i A') }} - Printed By: {{ Auth::user()->name }}</title>
...
<body class="d-flex flex-column min-vh-100" style="margin-left: 42px;">
  <script>
    ...
    function performAction() {
      window.location.href = "{{ route('stock.report') }}";
    }
  </script>
  <div class="invoice-container flex-fill" id="printableArea">
    ...
    <h3 style="text-align: center; margin: 0rem 0rem;"><b>Stock Information</b></h3>
    ...
    <table class=" invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size: 0.6rem;">
      ...
      <tbody>
        ...
        @php
        $buying_total = App\Models\SupplierPurcheseDetails::where('product_id',$item->id)->where('status','1')->sum('buying_qty');
        ...
        <tr>
          ...
          <td> {{ !empty($item['product']['brand']['name'])?$item['product']['brand']['name']:"N/A"  }} </td>
          <td> {{ !empty($item['product']['category']['name'])?$item['product']['category']['name']:"N/A" }} </td>
          <td> {{ $item->quantity  }}</td>
