...
<head>
    ...
    <title>Quotation #{{$invoice->id}} - Printed At: {{ now()->format('d-m-Y  h:i A') }} - Printed By: {{ Auth::user()->name }}</title>
...
<body class="d-flex flex-column min-vh-100" style="margin-left: 42px;">
    ...
    <div class="invoice-container flex-fill" id="printableArea">
        ...
        <div class="invoice-details" style="font-size: 1rem;  border-top: 2px solid #000;">
            <div class="mt-2" style="text-align: start; line-height: 1;">
                <p class="mb-1"><strong>{{ $payment->customer_id != -1 ? $payment['customer']['name'] : "পথচারি কাস্টমার"}}</strong></p>
            ...
            <div class="text-end mt-2">
                ...
                <p class="mb-1"><strong>Quotation #:</strong> {{ $invoice->invoice_no}}</p>
                @if ($invoice->wo_no)
                ...
                <tr style="border-bottom: 1px solid #000;">
                    ...
                    <td style="padding: 8px 12px; text-align: left;">
                        <span style=" color: black; font-weight: bold;">
                            <span style=" color: black; font-weight: bold;">{{ $details['product']['name'] }}</span>
                            <br>{!! $details['product']['description'] !!}
                    </td>
        ...
        <table class="invoice-table" style="width: 100%; border-collapse: collapse; margin-top: 0px; font-size:1rem;">
            ...
            <tbody>
                ...
                <tr style="font-weight: 400; padding:0;">
                    ...
                    <td class="text-end">
                        {{ number_format($payment->due_amount + $payment->paid_amount, 2) }}
                    </td>
                </tr>
                {{--
                    <tr style="font-weight: 400; padding:0;">
        ...
        <div class="total-section my-0 py-0">
            <div class="total-row" style="display: flex; justify-content: space-between; font-size: 0.7rem;">
                <span style=" width: 80%; display: inline-block;">
                    <b>IN WORDS: </b>
                    <strong>{{ strtoupper(convertToBangladeshiWords($payment->due_amount + $payment->paid_amount)) }}</strong>
                </span>
    ...
    <footer class="text-center mt-1 text-muted d-flex justify-content-center px-3"
        ...
        <small>ECS Engineering | Quotation No: {{$invoice->id}} | Generated By: {{ Auth::user()->name }} | At: {{ now()->format('d-m-Y  h:i A') }}</small>
    </footer>
</body>
