...
<head>
  ...
  <title> Return Invoice Preview</title>
  <!-- Fonts -->
      ...
      <div class="mx-auto" style="width:290px">
            ...
            <div class="col-12">
              ...
              <div class="invoice-header">
                <div class="invoice-col text-left">
                  <address>
                    <strong> Return Invoice #{{ $data['invoice_no'] }}</strong><br>
        ...
        <table id="pos-print-demo" class="table table-bordered text-left" style="width: calc(100% - 1px) !important">
          ...
          <tbody>
            ...
            @php $total_sum += $details->total; @endphp
            @endforeach
            @endif
            ...
            @php $total_retunr_sum += $details->total; @endphp
            @endforeach
        ...
        <table style="color: black!important; width: 100%!important">
          <tbody>
            <tr>
              <td colspan="2"></td>
              <td class="text-right">Total Price(R) (+Vat):</td>
              <td class="text-right">{{ number_format($total_retunr_sum,2) }} Tk</td>
            </tr>
            @if (!empty($finalProducts))
            <tr>
              <td colspan="2"></td>
              <td class="text-right">Total Price(E) (+Vat):</td>
              <td class="text-right">{{ number_format($total_sum,2) }} Tk</td>
            </tr>
            <tr>
              <td colspan="2"></td>
              <td class="text-right">Total Payable (+Vat):</td>
              <td class="text-right">
              @if($total_retunr_sum < $total_sum)
                {{ number_format($total_sum-$total_retunr_sum,2) }} Tk  
              @else
                {{ number_format(0,2) }} Tk
              @endif
              </td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td class="text-right">{{ number_format(abs($data['due_amount']), 2) }} Tk</td>
              @endif
            </tr>
          </tbody>
        </table>
        ...
        <h5 class="text-center">
          <strong>E - Exchange, R - Return</strong>
        <p class="text-center">
          <strong></strong>
          <span class="dashed-hr"></span>
      </div>
